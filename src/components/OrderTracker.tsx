...
import { CheckCircle2, Clock, ChefHat, Package, Truck, RefreshCcw, List, XCircle, Gift } from "lucide-react";
...
const OrderTracker = ({ order }: { order: Order }) => {
  const { addToCart, menuItems, user, setShowAuthModal, cancelOrder } = useApp();
  ...
  const hasFreeLineItem = order.items.some((line) => Number(line.price_at_order) === 0);
    ...
    >
      <div className="mb-3 flex items-center justify-between gap-3">
        ...
        <div className="text-right">
          ...
          {(order.isFreeOrder || hasFreeLineItem) && (
            <p className="mt-0.5 flex items-center justify-end gap-1 text-[11px] font-semibold text-secondary">
              <Gift className="h-3 w-3" /> FREE reward used
            </p>
          )}
        </div>
      </div>
      ...
      {order.status === "cancelled" ? (
        <div className="mb-3 flex items-center gap-2 rounded-lg border border-destructive/30 bg-destructive/5 p-2 text-xs font-semibold text-destructive">
          <XCircle className="h-4 w-4" /> This order was cancelled.
        </div>
      ) : order.status === "rejected" ? (
        <div className="mb-3 rounded-lg border border-destructive/30 bg-destructive/5 p-2 text-xs font-semibold text-destructive">
              ...
              <div key={step.key} className="flex flex-1 flex-col items-center">
                <div
                  className={`flex h-8 w-8 items-center justify-center rounded-full border-2 transition-colors ${
                    isComplete
                      ...
                      : isCurrent
                        ? "border-secondary bg-secondary/10 text-secondary animate-pulse-glow"
                ...
                <span
                  className={`mt-1 text-[10px] font-medium ${
                    isCurrent ? "text-secondary" : isComplete ? "text-primary" : "text-muted-foreground"
                  }`}
                >
                  {step.label}
                </span>
      ...
      <div className="mb-3 flex flex-wrap gap-2">
        ...
        {canCancel && (
          <button
            ...
            disabled={canceling}
            className="flex items-center gap-1 rounded-lg border border-destructive/60 bg-destructive/5 px-2.5 py-1.5 text-[11px] font-bold text-destructive transition-colors hover:bg-destructive hover:text-destructive-foreground disabled:cursor-not-allowed disabled:opacity-50"
          >
            <XCircle className="h-3.5 w-3.5" /> {canceling ? "Cancelling..." : "Cancel Order"}
          </button>
        )}
      </div>
      ...
      {showItems && (
        <div className="mb-3 space-y-2 rounded-lg border bg-muted/20 p-2.5">
          {order.items.map((line, idx) => {
            ...
            const isFreeLine = Number(line.price_at_order) === 0;
            return (
              <div
                key={`${line.menu_item?.id || line.menu_item?.name || idx}-${idx}`}
              ...
              >
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-muted-foreground">
                    Qty {line.quantity} • {isFreeLine ? "FREE" : `₹${line.price_at_order}`}
                  </p>
                </div>
                <button
